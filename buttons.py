...
def location_kb():
    ...
    Location = KeyboardButton('Поделится локатций', request_location=True)
...
#Кнопки для подверждение заказа
def get_accept_kb():
    kb = ReplyKeyboardMarkup(resize_keyboard=True)

    yes = KeyboardButton('Подвердить')
...
def get_cart_kb():
    ...
    order = InlineKeyboardButton('Оформить заказ', callback_data='clear cart')
    back = InlineKeyboardButton('Назад', callback_data='clear cart')
